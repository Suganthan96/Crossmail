...
const Shuffle = forwardRef<HTMLElement, ShuffleProps>(({
  ...
  // External trigger effect
  useEffect(() => {
    if (externalTrigger && externalTrigger > 0 && ready && ref.current) {
      // Small delay to ensure both components trigger together
      const timeout = setTimeout(() => {
        if (!playingRef.current && ref.current) {
          // Trigger animation manually
          const event = new Event('externalTrigger');
  ...
  useGSAP(
    () => {
      ...
      const armHover = () => {
        if (!triggerOnHover && !externalTrigger && !ref.current) return;
        removeHover();
        ...
        if (externalTrigger) {
          ...
          ref.current!.addEventListener('externalTrigger', externalHandler);
          
          // Store reference for cleanup
          const currentRef = ref.current;
          return () => currentRef?.removeEventListener('externalTrigger', externalHandler);
        }
      };
      ...
      const setupAutoTrigger = () => {
        ...
        autoTriggerTimeoutRef.current = setTimeout(() => {
          if (playingRef.current || !ref.current) return;
      ...
      const st = ScrollTrigger.create({
        ...
        onEnter: () => {
          create();
          if (autoTrigger) {
            // Start auto-trigger after initial animation
            setTimeout(setupAutoTrigger, autoTriggerInterval);
          }
        }
      });
